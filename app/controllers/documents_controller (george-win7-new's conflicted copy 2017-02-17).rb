...
#   params[:document][:user_ids] ||= []
  ...
  def search_results 
    ...
    @documents = Document.where("(documents.subject LIKE ? OR documents.target_protocol LIKE ? OR documents.source_protocol LIKE ?)",
          condition, condition, condition).order("documents.dispatch_stage_id, documents.dispatch_deadline").page(params[:page]).per(20)
  end 
...
# ΟRIGINAL END ---------------------------------------
  ...
  def archived
    @documents = Document.archived.order('target_date DESC').page(params[:page]).per(200)
  end
